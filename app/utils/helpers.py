...
def get_market_timing() -> Dict[str, Any]:
    ...
    return {
        ...
        "time_to_close": (
            _get_time_to_close(now) if (is_market_hours and is_market_day) else None
        ),
    }
...
def calculate_ipo_metrics(ipo_data: Dict) -> Dict[str, Any]:
    ...
    if subscription_times and subscription_times.replace(".", "").isdigit():
        ...
        metrics["subscription_status"] = (
            "Oversubscribed" if sub_rate > 1.0 else "Undersubscribed"
        )
    ...
    if price_range["min_price"] and price_range["max_price"]:
        metrics["price_band"] = (
            f"₹{price_range['min_price']:.0f} - ₹{price_range['max_price']:.0f}"
        )
    ...
    if start_date and end_date:
        ...
        metrics["ipo_duration"] = (
            days_to_end - days_to_start if days_to_end > days_to_start else 0
        )
