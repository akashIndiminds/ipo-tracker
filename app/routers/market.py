...
try:
    ...
    logger.error(f"Import error in Market router: {e}")
    # Create fallback classes if imports fail
    class NSEService:
        async def get_market_indices(self): return []
        async def get_market_status(self): return []
    
    class DataProcessor:
        @staticmethod
        def clean_market_data(data): return data
        @staticmethod
        def filter_major_indices(data): return data
        @staticmethod
        def calculate_market_sentiment(data): return {"sentiment": "neutral", "sentiment_score": 50}
        @staticmethod
        def get_market_highlights(data): return {}
    ...
    try:
        ...
        dashboard = {
            ...
            "dashboard": {
                ...
                "statistics": {
                    "total_indices": sentiment_data.get('total_count', 0),
                    "positive_indices": sentiment_data.get('positive_count', 0),
                    "negative_indices": sentiment_data.get('negative_count', 0),
                    "neutral_indices": sentiment_data.get('total_count', 0) - sentiment_data.get('positive_count', 0) - sentiment_data.get('negative_count', 0)
                },
...
async def get_market_overview():
    ...
    try:
        ...
        # Find key indices
        nifty_50 = next((idx for idx in indices_clean if idx.get('index_name') == 'NIFTY 50'), None)
        bank_nifty = next((idx for idx in indices_clean if idx.get('index_name') == 'NIFTY BANK'), None)
...
def _get_key_levels(nifty_data: dict) -> dict:
    ...
    return {
        ...
        "day_high": high,
        "day_low": low,
        "support_level": round(low * 0.995, 2) if low else 0,  # Approximate support
        "resistance_level": round(high * 1.005, 2) if high else 0,  # Approximate resistance
        "range": round(high - low, 2) if high and low else 0
    }
