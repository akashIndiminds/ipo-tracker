    ...
    try:
        ...
        if current_ipos:
            ...
            is_real_data = not any(
                "Demo" in str(item.get("status", "")) for item in current_ipos
            )
        ...
        if indices:
            ...
            # Show NIFTY 50
            nifty = next(
                (idx for idx in indices if "NIFTY 50" in idx.get("indexName", "")), None
            )
            if nifty:
                print(f"   NIFTY 50: {nifty.get('last', 'N/A')}")
                print(
                    f"   Change: {nifty.get('change', 'N/A')} ({nifty.get('percChange', 'N/A')}%)"
                )
        else:
